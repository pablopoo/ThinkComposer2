...
It is still not a complete legacy replacement only where the remaining work depends on product/release decisions: real signing certificate provisioning, installer/update hosting, and licensing policy.
...
- Output: SVG/HTML/PDF export, full document HTML/PDF report, HTML presentation deck, template-based file generation, and full-document printable HTML preview.
