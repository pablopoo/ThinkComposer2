...
namespace DotLiquid
{
    public class Context
    {
        ...
        {
            switch (key.ToLower())  //$ ToLower() supports non-ruby language case style
            {
                case null:
                case "nil":
                case "null":
        ...
        private static object Liquidize(object obj)
        {
            ...
            if (obj is IEnumerable) //$ Moved to avoid try a Table (ContainedDetail) as IEnumerable. Previously, this was just between 'string' and '...IsPrimitive'.
                return obj;
            if (obj.GetType().GetCustomAttributes(typeof(LiquidTypeAttribute), false).Any())   //$? Also gets attributes of ancestors
            {
                var attr = (LiquidTypeAttribute)obj.GetType().GetCustomAttributes(typeof(LiquidTypeAttribute), false).First();   //$? Also gets attributes of ancestors
